...
DIRECTORY=$(pwd)/l2sces

MANAGED_1_BEARER_TOKEN=$(kubectl --kubeconfig ${DIRECTORY}/managed-1/kubeconfig create token -n l2sces-system l2sces-controller-manager --duration 1000h)
MANAGED_2_BEARER_TOKEN=$(kubectl --kubeconfig ${DIRECTORY}/managed-2/kubeconfig create token -n l2sces-system l2sces-controller-manager --duration 1000h)
