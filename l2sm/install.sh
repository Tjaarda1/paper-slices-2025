...
L2SM_KEYS_DIR="$LOCALCONFIG/l2sm"
...
kubectl --kubeconfig "$K8S_DIR/kubeconfig-l2sm-control.yaml" \
...
kubectl --kubeconfig "$K8S_DIR/kubeconfig-l2sm-managed-1.yaml" \
...
"$L2SM_MD_BIN" --namespace l2sm-system \
  --kubeconfig "$K8S_DIR/kubeconfig-l2sm-control.yaml" \
  --clustername l2sm-managed-1 \
  "$L2SM_KEYS_DIR/cluster-managed-1.key"

echo "==> Extracting CA from managed-2"
kubectl --kubeconfig "$K8S_DIR/kubeconfig-l2sm-managed-2.yaml" \
  config view -o jsonpath='{.clusters[0].cluster.certificate-authority-data}' --raw \
...
kubectl --kubeconfig "$K8S_DIR/kubeconfig-l2sm-managed-1.yaml" wait --for=condition=Ready pods --all -A  --timeout=300s
kubectl --kubeconfig "$K8S_DIR/kubeconfig-l2sm-managed-2.yaml" wait --for=condition=Ready pods --all -A  --timeout=300s
