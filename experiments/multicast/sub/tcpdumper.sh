...
NMAP_FLAGS="${NMAP_FLAGS:--sn -PE -e eth0 -T4 --max-retries 1 --max-rtt-timeout 500ms --initial-rtt-timeout 250ms}"
...
flush_arp "$CTX1" toolbox-0 
flush_arp "$CTX2" toolbox-0 
flush_arp "$CTX2" toolbox-1 
...
ensure_tools "$CTX1" toolbox-0 
ensure_tools "$CTX2" toolbox-0 
ensure_tools "$CTX2" toolbox-1

# --- Kick off captures on all 4 pods -----------------------------------------

start_capture "$CTX1" toolbox-0  "run1"
start_capture "$CTX2" toolbox-0  "run1"
start_capture "$CTX2" toolbox-1  "run1"
...
kubectl --context "$CTX1" -n "$NS" exec toolbox-0 -- sh -lc \
  "ping -b -I eth0  -i 0.2 -w ${PING_DURATION} 242.0.255.255" | tee "$PING_LOG" || true

# --- Multicast test (servers first, then client) ------------------------------
MULTICAST_GROUP="${MULTICAST_GROUP:-239.94.1.1}"
SRC_IF="eth0"

# discover client source IP on eth0 (CTX1/ping)
CLIENT_SRC_IP="$(
  kubectl --context "$CTX1" -n "$NS" exec toolbox-0 -- sh -lc \
...
(kubectl --context "$CTX2" -n "$NS" exec toolbox-0 -- sh -lc \
...
(kubectl --context "$CTX2" -n "$NS" exec toolbox-1 -- sh -lc \
  "iperf -s -u -B ${MULTICAST_GROUP} -i 1" \
...
copy_pcaps "$CTX1" toolbox-0  
copy_pcaps "$CTX2" toolbox-0  
copy_pcaps "$CTX2" toolbox-1  
