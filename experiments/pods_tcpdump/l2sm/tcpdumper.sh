...
echo "[i] (${CTX1}/ping) pinging ${TARGET_IP_POD0_CTX2} for ${PING_DURATION}s…"
# kubectl --context "$CTX1" -n "$NS" exec ping -- sh -lc \
#   "ping -i 0.2 -w ${PING_DURATION} ${TARGET_IP_POD0_CTX2}" | tee "$PING_LOG" || true

declare -i now=$PING_START
if (( NMAP_START > now )); then sleep $((NMAP_START-now)); fi
NMAP_LOG="${OUTDIR}/nmap_ping_${CTX1}.log"
# kubectl --context "$CTX1" -n "$NS" exec ping -- sh -lc \
